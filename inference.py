...
# Fallback: read HF_TOKEN from env.txt if not set in environment
if HF_TOKEN is None:
    try:
        with open(os.path.join(os.path.dirname(__file__), "env.txt")) as _f:
            for _line in _f:
                _line = _line.strip()
...
def run_task(task_id: str, use_llm: bool = True) -> Dict[str, Any]:
    ...
    Always emits exactly one [START] and one [END] via try/finally.
    ...
    try:
        ...
        try:
            grade   = env_grade()
            score   = clamp(float(grade.get("score", MIN_SCORE)))
    ...
    return {
        ...
        "final_score":  score,
        "steps":        len(rewards),
        "total_reward": clamp(round(sum(rewards), 2)),
...
def main():
    ...
    # No separate health check — if env is unreachable, each run_task handles it
    # gracefully with its own [START]/[END] pair (avoids orphan [END] with no [START])
    ...
    scores  = [clamp(float(r.get("final_score", MIN_SCORE))) for r in results.values()]
    avg     = clamp(round(sum(scores) / len(scores), 2))
    ...
    output = {
        ...
        "scores":          {t: clamp(float(r.get("final_score", MIN_SCORE))) for t, r in results.items()},
        "average_score":   avg,
