...
def run_task(task_id: str, use_llm: bool = True) -> Dict[str, Any]:
    ...
    for step_n in range(max_steps):
        ...
        try:
            ...
            log_step(step_n + 1, action.get("action_type", "unknown"), 0.01, True, str(ex)[:120])
            break
        ...
        log_step(step_n + 1, action.get("action_type", "unknown"), rv, bool(done), last_error)
...
def main():
    ...
    for task_id in TASKS:
        try:
            ...
            traceback.print_exc()
            results[task_id] = {"task_id": task_id, "error": str(e), "final_score": MIN_SCORE, "steps": 0}
            log_end(success=False, steps=0, rewards=[])
